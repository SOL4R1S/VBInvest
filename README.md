...
`VBinvest.command`는 기본적으로 서버를 백그라운드에서 실행하고 실행에 사용된 터미널 창을 자동으로 닫습니다.
로그는 `~/Library/Logs/VBinvest/launcher.log`에 기록됩니다.
터미널 창을 남기려면 `VBINVEST_KEEP_TERMINAL=1 ./VBinvest.command`, 터미널에 붙여 디버깅하려면 `VBINVEST_FOREGROUND=1 ./VBinvest.command`로 실행하세요.
앱은 웹 화면의 `종료` 버튼으로 닫을 수 있고, 브라우저 탭을 닫을 때도 로컬 종료 요청을 best-effort로 전송합니다.
...
현재 반영된 주요 기능은 다음과 같습니다.

- 로컬 우선 실행: macOS/Windows 런처, 자동 빈 포트 선택, macOS 백그라운드 실행, 웹 UI 종료 버튼
- 첫 실행 설정: SQLite/PostgreSQL, Obsidian Vault, OpenDART, AI API/Local LLM 설정
- 시장 데이터: 앱 시작 시 관심 종목 가격 백필 및 증분 갱신, yfinance 기반 가격 수집, 5년 초기 백필, 최근 1260거래일 대시보드 조회
- 지표/차트: RSI14, MA5/20/50/120, 캔들/라인 전환, 보조 지표 패널, 이동평균 색상 범례, 휠 줌과 드래그 이동
- 종목 관리: 관심 그룹, 티커 유효성 검증, 이름/티커 자동완성 검색, 시작 시 종목 카탈로그 갱신
- 리포트: 온디맨드 리포트 생성, DB 가격/지표/뉴스/공시 기반 분석, Obsidian Markdown 내보내기
- UX: 시장 데이터 갱신 중 진행률, 경과 시간, 예상 남은 시간 표시
- **포트폴리오 수익률 추적**: 매수/매도 거래 기록, 실현·미실현 수익률, 포트폴리오 요약 대시보드
- **알림 시스템**: 가격 알림·스케줄러 결과·리서치 완료 알림, 종 아이콘 배지 + 드롭다운 + 브라우저 Notification
- **CSV/데이터 내보내기**: 포트폴리오·거래이력·워치리스트 가격·리서치 리포트를 CSV/JSON/Markdown으로 다운로드
- **AI 리서치 품질 개선**: 섹터별 프롬프트 템플릿, 소스 인용 검증, 신뢰도 스코어
- **플러그인 데이터 소스**: PriceSourceRegistry 기반 확장 가능 가격 소스 (Yahoo Chart, yfinance, Stooq 내장)
- **커뮤니티 템플릿**: 사용자 정의 AI 프롬프트 템플릿 CRUD API
- **선택적 서버 모드**: Docker Compose로 앱 + PostgreSQL 원클릭 배포 (`make docker-up`)

### 로컬 LLM 테스트 기록

아래 기록은 특정 사용자 환경에서의 실사용 확인 결과입니다. 로컬 모델은 모델 크기, 컨텍스트 길이, Ollama 버전, 시스템 메모리 상태에 따라 결과가 달라질 수 있습니다.

| 항목 | 내용 |
| --- | --- |
| 테스트 장비 | Mac mini M4, 24GB RAM |
| 런타임 | Ollama 로컬 OpenAI-compatible endpoint |
| 정상 작동 확인 | `gemma4:e4b-it-q4_K_M` |
| 오류 발생 확인 | `qwen3.5:2b`, `hf.co/NidAll/supergemma4-e4b-abilterated-Q4_K_M-GGUF:Q4_K_M` |
| 확인 결과 | `gemma4:e4b-it-q4_K_M` 설정에서 온디맨드 리포트 생성과 Obsidian Markdown 저장 흐름이 정상 동작하는 것을 확인했습니다. 오류가 발생한 모델은 로컬 모델 응답 형식/컨텍스트/호환성 문제로 리포트 생성이 실패할 수 있어 다른 모델로 교체하는 것을 권장합니다. |
...
GitHub에서 렌더링되는 안내 문구만 제공하며 가짜 이미지는 사용하지 않습니다.
...
예약 사전 생성은 기본 비활성화입니다. 리포트는 기본적으로 종목 화면의 `리포트 발행` 버튼을 눌렀을 때 온디맨드로 생성됩니다.
