...
from scripts.lib.config import ConfigError, load_opendart_api_key, serialize_report_run_summary
...
def ingest_assets(
    ...
    *,
    fetch_history: Callable[..., object] | None = None,
    ...
    try:
        for asset in assets:
            symbol = asset["symbol"]
            try:
                window = refresh_windows.get(int(asset["asset_id"]))
                if window is not None and window.persist_start > window.end:
                    continue
                prices = _fetch_for_asset(symbol, options, fetch_history, window)
                enriched = add_indicators(prices)
                persist_frame = filter_history_for_persistence(enriched, window)
                built_prices = build_price_rows(asset["asset_id"], persist_frame, fetched_at=options.fetched_at)
                built_indicators = build_indicator_rows(asset["asset_id"], persist_frame)
                ...
                if options.include_news:
                    source_result = collect_asset_sources(
                        asset,
                    ...
                    provider_disabled.extend(source_result.provider_disabled)
                    news_items += source_result.news_items
                    disclosures += source_result.disclosures
...
def _fetch_for_asset(
    symbol: str,
    options: IngestOptions,
    fetch_history: Callable[..., object] | None,
    window: PriceRefreshWindow | None,
):
        ...
        try:
            if fetch_history is None:
                return fetch_price_history(
                    ...
                    no_network=options.no_network,
                    start_date=window.fetch_start if window else None,
                    end_date=window.end if window else None,
                )
            if window is None:
                return fetch_history(symbol)
            return fetch_history(symbol, start_date=window.fetch_start, end_date=window.end)
...
def main() -> int:
    ...
    summary = f"assets={len(assets)} price_rows={result.price_rows} indicator_rows={result.indicator_rows} failed={len(result.failures)}"
    run_id = None
    if db is not None:
